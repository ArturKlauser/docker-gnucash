...
*   **Secure Access**: Web interface is forced to HTTPS and optionally secured
    by a login page.
*   **Local sign-in**: Simple authentication against a local user database can
    be enabled.
*   **Oauth2 Proxy**: Authentication against an SSO identity provider (Google,
    GitHub, any OIDC provider, etc.) can be enabled.
...
1.  Access the interface at [https://localhost:5800](https://localhost:5800).
    *  Note: Since a self-signed certificate is generated by default, you will
       ...
       [How to use an existing SSL certificate](#how-to-use-an-existing-ssl-certificate-eg-from-lets-encrypt)
       for details.
...
| `OAUTH2_PROXY` | Enable OAuth2 Proxy support. Overrides WEB_AUTHENTICATION if set. | false (optional) |
...
>     this option).
> 2.  Set the `OAUTH2_PROXY` environment variable to `1` in your
...
> provider.
>
> **Important:** The default configuration is not fully functional out-of-the-box.
> You need to edit `/config/oauth2-proxy.cfg` to configure your specific OAuth2
> provider (client ID, client secret, etc.) and other settings. Look for sections
> marked `CHANGE THIS`.
>
...
1.  At the time of writing (Feb. 2026), the documentation adds ~90 MB
    (~8%) to the uncompressed docker image size. The majority of the image size of ~1.1 GB is the
    `gnucash` package itself and its dependencies.
2.  At the time of this writing (Feb. 2026), oauth2-proxy adds ~27 MB to the uncompressed docker image size.
