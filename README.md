...
1.  Access the interface at [https://localhost:5800](https://localhost:5800).
    *  Note: Since a self-signed certificate is generated by default, you will
       ...
       certificate in `/config/certs`. See
       [How to use an existing SSL certificate](#existing-ssl-cert-setup)
       for details.
...
* **User Authentication**: Optionally, users can be required to authenticate
  before accessing the application. This can be done with a local user database
  or via an SSO OAuth2 identity provider (Google, GitHub, OIDC server, etc.)
...
>     container.
> * **config**: The `*.gnucash` file above contains the main "book" data, such
>   as accounts names and their transactions. Additional information, however,
>   like UI settings or saved reports, is stored in separate configuration
>   files.  Those should be copied to the container's `/config` directory.
>   Depending on the type of host system, you'll find those files in different
>   locations:
>   * Linux host:
...
>
>   * MacOS host:
>     * Configuration location: Open the GnuCash app and select "Gnucash ->
>       About Gnucash". The info box that pops up shows `GNC_USERDATA_DIR` and
...
>    this option).
> 2. Set the environment variable `OAUTH2_PROXY=1`in your `docker-compose.yml`
>    or `docker run` command.
> 3. Set the environment variable `SECURE_CONNECTION=1`, as OAuth2 Proxy
>    requires HTTPS. (This is the default.)
>
...
> sample default configuration file will be created that uses GitHub as identity
> provider.
>
> **Important:** The default configuration is not fully functional
> out-of-the-box.  You need to edit `/config/oauth2-proxy.cfg` to configure your
> specific OAuth2 provider (client ID, client secret, etc.) and other settings.
> Look for sections marked `CHANGE THIS`.
>
> If a local password database file is found at `/config/oauth2-proxy-htpasswd`
> it is used to also allow local users in this database to log in. Entries in
> the file need to be created with `htpasswd -B`. If on container startup both
> environment variables `WEB_AUTHENTICATION_USERNAME` and
> `WEB_AUTHENTICATION_PASSWORD` are set, that user is added to the password
> file.
> </details>
